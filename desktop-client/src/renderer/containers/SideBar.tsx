...
const SideBarButton = ({ icon, text, componentName, isOpen, tab, disabled, history }: buttonProps) => {
	const statusColor = disabled ? "rgba(124, 124, 124, 0.548)" : "white"
	return (
		<div className={`sidebar-button${disabled ? "" : "-enabled"}`}
			style={tab.type.name === componentName ? {
		...
		>
			<FontAwesomeIcon icon={icon} size="lg" className="sidebar-button-icon"
				fixedWidth
				color={statusColor}
			/>
			{isOpen ?
