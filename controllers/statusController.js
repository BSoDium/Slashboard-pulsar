  ...
  try {
    ...
    if (
      ...
      for (let i = 0; i < cores.length; i++) {
        // we assume the cpu count is constant
        const core = cores[i];
        ...
        hardware: {
          cpu: {
            ...
            global: {
              ...
              speed:
                cores.reduce((acc, cur) => acc + cur.speed, 0) / cores.length,
              load:
                cores.reduce((acc, cur) => acc + cur.load, 0) / cores.length,
            },
          },
          memory: {
            total: os.totalmem(),
...
exports.getOverview = async (req, res) => {
  try {
    ...
    let checkAuth = Citadel.checkJWT(
      token,
      appContext.hostname,
      appContext.serverKey
    );
